...
add_action( 'bbp_new_reply',    'mjj_bbp_notify_subscribers_cron', 11, 5 );
// the hook in the cron is the notify subscription function itself
add_action( 'mjj_bbp_notify_subscribers_callback',    'mjj_bbp_notify_subscribers', 11, 5 );

//add the one click unsubscribe on the topic itself
add_action( 'bbp_template_before_single_topic', 'mjj_bbp_unsubscribe_notice' );

// the cron job setup
function mjj_bbp_notify_subscribers_cron( $reply_id = 0, $topic_id = 0, $forum_id = 0, $anonymous_data = false, $reply_author = 0 ){
...
function mjj_bbp_notify_subscribers( $reply_id = 0, $topic_id = 0, $forum_id = 0, $anonymous_data = false, $reply_author = 0 ) {
	...
	foreach ( (array) $user_ids as $user_id ) {
		...
		$nn_link 	   = not_a_nonce( $user_id, $topic_id );
...
function mjj_bbp_unsubscribe_notice(){
	...
	$check = not_a_nonce( $user_id, $topic_id );
